...
class TutorialPanel extends HTMLElement {
	...
	launch() {
		...
		if (this.tutorials.length == 1) {
			...
			this.tutorials.push("By default, the right side of the screen shows the \"home\" tab (a house icon). This panel will show your nation's name, color, and icon. In future turns it will display important notifications about what happened during the resolution of the previous turn. For now, take the opportunity to locate your nation on the map (you may have to scroll to bring it into view). You can double-check that a region on map is controlled by your nation by clicking on it and matching the national color, insignia, and name in the pop-up to the ones in the home tab.</p>");
			...
			this.tutorials.push("Click the barley icon to go to the \"land\" tab. This tab provides economic controls that you can use to balance your budget, happiness, and food requirements. On the first turn, your regions have balanced food, so there is no need to move food around your nation. On future turns, it may be wise to evacuate food stocks from a region that is about to be invaded: though moving food is expensive, starvation is almost always worse.");
