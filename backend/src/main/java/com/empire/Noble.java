	...
	enum Type {
		...
		WEDDING("%NOBLENAME%'s daughter is being wed in %REGIONNAME% and they request your presence as ruler.", "The wedding of the daughter of %NOBLENAME% was a spectacular affair. It is rare for the nobility to find much joy in their political marriage, but in this case the couple's obvious love for one another warmed your heart to witness. %NOBLENAME% could not thank you enough for attending, and swore never to forget this day.", "The wedding %NOBLENAME% invited you to in %REGIONNAME% has taken place without you and %NOBLENAME% is very cross.") {
			...
			boolean isCreateable(World w, Region r, Map<Army, Character> leaders, Map<Region, ArrayList<Character>> governors, Set<Region> builds, Set<Region> templeBuilds, Map<String, Double> rationing, Set<String> lastStands, int inspires) {
				...
				for (Character c : w.characters) {
					if (c.kingdom.equals(r.getKingdom()) && !c.captive() && c.hasTag(Character.Tag.RULER) && closeRegions.contains(c.location)) {
						return true;
					}
				}
				return false;
		...
		OVERWHELMED("%NOBLENAME% can't keep up with the stresses of managing a %REGIONNAME% and has asked for one of our agents to assist by governing it for a week.", "Assisted by our governance of %REGIONNAME%, %NOBLENAME% has gotten back on their feet - and picked up a trick or two!", "%NOBLENAME% has resolved their troubles %REGIONNAME% but acquired a habit of accepting wastefulness.") {
			...
			boolean isCreateable(World w, Region r, Map<Army, Character> leaders, Map<Region, ArrayList<Character>> governors, Set<Region> builds, Set<Region> templeBuilds, Map<String, Double> rationing, Set<String> lastStands, int inspires) {
				Set<Integer> closeRegions = r.getCloseRegionIds(w, Constants.nobleCrisisFrequency);
				for (Character c : w.characters) {
					if (c.kingdom.equals(r.getKingdom()) && !c.captive() && closeRegions.contains(c.location)) {
						return true;
					}
				}
				return false;
		...
		abstract boolean isSolved(World w, Region r, Map<Army, Character> leaders, Map<Region, ArrayList<Character>> governors, Set<Region> builds, Set<Region> templeBuilds, Map<String, Double> rationing, Set<String> lastStands, int inspires);
		abstract boolean isCreateable(World w, Region r, Map<Army, Character> leaders, Map<Region, ArrayList<Character>> governors, Set<Region> builds, Set<Region> templeBuilds, Map<String, Double> rationing, Set<String> lastStands, int inspires);
