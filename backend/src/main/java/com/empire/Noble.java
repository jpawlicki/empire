...
class Noble extends RulesObject {
	 ...
	 */	
	Optional<Notification> resolveCrisis(World w, Region r, Map<Army, Character> leaders, Set<Region> builds, Set<Region> templeBuilds, Map<String, Double> rationing, int inspires) {
		if (crisis.type.isSolved(w, r, leaders, builds, templeBuilds, rationing, inspires)) {
	 ...
	 * @return a notification that should be added to the world.
	 */
	Optional<Notification> createCrisis(World w, Region r, Map<Army, Character> leaders, Set<Region> builds, Set<Region> templeBuilds, Map<String, Double> rationing, int inspires) {
		...
		for (Crisis.Type type : Crisis.Type.values()) {
			if (!type.isCreateable(w, r, getRules(), leaders, builds, templeBuilds, rationing, inspires)) continue;
			if (type.isSolved(w, r, leaders, builds, templeBuilds, rationing, inspires)) continue;
...
final class Crisis {
	...
	enum Type {
		NONE("", "", "") {
			...
			@Override
			boolean isCreateable(World w, Region r, Rules rules, Map<Army, Character> leaders, Set<Region> builds, Set<Region> templeBuilds, Map<String, Double> rationing, int inspires) {
				return false;
			}
		},
		...
		NOBILITY("%NOBLENAME% of %REGIONNAME% has many friends who they believe ought to be raised to the nobility. Install nobles in all regions under your control.", "%NOBLENAME% is pleased that you have installed their friends as nobles.", "%NOBLENAME% is upset that you have failed to install additional nobles in your lands.") {
			@Override
			boolean isSolved(World w, Region r, Map<Army, Character> leaders, Set<Region> builds, Set<Region> templeBuilds, Map<String, Double> rationing, int inspires) {
				return w.regions.stream().filter(rr -> rr.isLand() && rr.getKingdom().equals(r.getKingdom()) && !rr.hasNoble()).count() == 0;
			}
		...
		PATROL("%NOBLENAME% is concerned about security and asks you to position a 1000 soldiers in %REGIONNAME%.", "%NOBLENAME% is pleased that you have taken their advice and positioned forces in %REGIONNAME%.", "%NOBLENAME% is upset that you have ignored their advice to bring troops to %REGIONNAME%.") {
			@Override
			boolean isSolved(World w, Region r, Map<Army, Character> leaders, Set<Region> builds, Set<Region> templeBuilds, Map<String, Double> rationing, int inspires) {
				for (Army a : w.armies) if (a.isArmy() && a.getEffectiveSize() >= 1000 && a.location == r.id && a.kingdom.equals(r.getKingdom())) return true;
				return false;
			}
		...
		RECESSION("%NOBLENAME% of %REGIONNAME% is concerned about the national treasury, and advises building it up to at least 300 gold.", "%NOBLENAME% is pleased that you have taken their advice and increased your national treasury.", "%NOBLENAME% is upset that you have ignored their advice to increase the treasury.") {
			@Override
			boolean isSolved(World w, Region r, Map<Army, Character> leaders, Set<Region> builds, Set<Region> templeBuilds, Map<String, Double> rationing, int inspires) {
				return w.getNation(r.getKingdom()).gold >= 300;
			}
		...
		abstract boolean isSolved(World w, Region r, Map<Army, Character> leaders, Set<Region> builds, Set<Region> templeBuilds, Map<String, Double> rationing, int inspires);
		abstract boolean isCreateable(World w, Region r, Rules rules, Map<Army, Character> leaders, Set<Region> builds, Set<Region> templeBuilds, Map<String, Double> rationing, int inspires);
